...
# tensors for streaming (soe-prepare). Idempotent + single-instance.
set -eu
...
if [ -f "$OUT" ]; then
    ...
    elif [ "$SZ" -lt "$EXPECTED_SIZE" ]; then
        echo "[q4] resuming partial ($SZ / $EXPECTED_SIZE bytes)"
    else
        echo "[q4] removing oversized/corrupt partial"
        rm -f "$OUT"
    fi
fi
